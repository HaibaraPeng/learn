...
import com.roc.shop.common.constants.OauthCacheNames;
import com.roc.shop.common.enums.YamiHttpStatus;
import com.roc.shop.common.exception.YamiShopBindException;
import com.roc.shop.common.serializer.redis.KryoRedisSerializer;
import com.roc.shop.common.util.PrincipalUtil;
import com.roc.shop.security.common.bo.TokenInfoBO;
...
public class TokenStore {
    ...
    public TokenStore(RedisTemplate<String, Object> redisTemplate, StringRedisTemplate stringRedisTemplate) {
    ...
    public TokenInfoBO storeAccessToken(UserInfoInTokenBO userInfoInToken) {
        ...
        // 一个用户会登陆很多次，每次登陆的token都会存在uid_to_access里面
        // 但是每次保存都会更新这个key的时间，而key里面的token有可能过期，过期就要移除掉
    ...
    public TokenInfoBO refreshToken(String refreshToken) {
        ...
        if (StrUtil.isBlank(accessToken)) {
            throw new YamiShopBindException(YamiHttpStatus.UNAUTHORIZED, "refreshToke 已过期");
        }
    ...
    private String decryptToken(String data) {
        ...
        try {
            ...
            // 创建token的时间，token的使用时效性，防止攻击者通过一堆的尝试找到aes的密码，虽然aes是目前集合最好的加密算法
